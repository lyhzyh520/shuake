...
namespace DAL
{
    ...
    public class DBHelper : IDisposable
    {
        ...
        public static SqlConnection Connectionstrings
        {
            get
            {
                constr = ConfigurationManager.ConnectionStrings["cnnStr"].ToString();
        ...
        public static T ExecuteScalar<T>(string cmdTxt, CommandType cmdtype, params SqlParameter[] pars)
        {
            using (cmd = new SqlCommand(cmdTxt, Connectionstrings))
            {
                cmd.CommandType = cmdtype;
                cmd.Parameters.AddRange(pars);
                cmd.Parameters.Add("@result", SqlDbType.Char).Direction = ParameterDirection.Output;
                T result = (T)cmd.ExecuteScalar();
                conn.Close();
                return result;
            }
        ...
        //public static string ExecuteScalar<string>(string cmdTxt,CommandType cmdtype,params SqlParameter[] pars)
        //{
        //    using(cmd=new SqlCommand(cmdTxt,Connectionstrings))
        //    {
        //        using (cmd = new SqlCommand(cmdTxt, Connectionstrings))
